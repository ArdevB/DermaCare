...
import { upsertReview, deleteReview as deleteReviewRequest, getMyReview } from "@/lib/reviewActions";
import { getErrorMessage } from "@/lib/api";
import { formatDate, initials } from "@/lib/format";
import { StarRatingDisplay, StarRatingInput } from "@/components/shop/StarRating";

export function ReviewsSection({ productId, ratingsSummary, onChanged }) {
  const { user } = useShop();
  const [page, setPage] = useState(1);
  const { data, isLoading, isError, refetch } = useApiQuery(`/reviews/${productId}`, { page, limit: 5 });
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <StarRatingDisplay value={ratingsSummary?.average ?? 0} size="h-5 w-5" />
          <span className="text-sm text-gray-600">
            {(ratingsSummary?.average ?? 0).toFixed(1)} out of 5 &middot; {ratingsSummary?.count ?? 0}{" "}
            review{ratingsSummary?.count === 1 ? "" : "s"}
          </span>
        </div>
      ...
      {formOpen && (
        <form onSubmit={handleSubmit} className="mt-4 rounded-lg border p-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Your rating</p>
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            {myReview && (
              ...
              >
                <Trash2 className="h-3.5 w-3.5" /> {deleting ? "Deleting..." : "Delete review"}
              </button>
            )}
          </div>
        </form>
      ...
      <div className="mt-6 divide-y">
        {isError && (
          <p className="py-6 text-center text-sm text-red-500">Couldn&apos;t load reviews.</p>
        )}
        {isLoading ? (
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 flex items-center gap-2">
                    <StarRatingDisplay value={review.rating} size="h-3.5 w-3.5" />
